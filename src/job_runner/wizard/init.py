"""Job Runner first-time setup wizard.

Interactive flow that creates ~/.job_runner/ with:
...
from job_runner.resume import extract_resume_text_with_source
...
def _setup_profile() -> dict:
    ...
    console.print(Panel("[bold]Step 2: Profile[/bold]\nTell Job Runner about yourself. This powers scoring, tailoring, and auto-fill."))
...
def _setup_searches() -> None:
    ...
    # Build YAML content
    lines = [
        "# Job Runner search configuration",
...
def _setup_ai_features() -> None:
    ...
    if not Confirm.ask("Enable AI scoring and resume tailoring?", default=True):
        console.print("[dim]Discovery-only mode. You can configure AI later with [bold]job_runner init[/bold].[/dim]")
        return
    ...
    env_lines = ["# Job Runner configuration", ""]
...
def _setup_auto_apply() -> None:
    ...
    console.print(Panel(
        ...
        "Job Runner can autonomously fill and submit job applications\n"
        "using Claude Code as the browser agent."
    ))

    if not Confirm.ask("Enable autonomous job applications?", default=True):
        console.print("[dim]You can apply manually using the tailored resumes Job Runner generates.[/dim]")
        return
    ...
    if Confirm.ask("Configure CapSolver API key? (optional)", default=False):
        ...
        if ENV_PATH.exists():
            ...
            ENV_PATH.write_text(f"# Job Runner configuration\nCAPSOLVER_API_KEY={capsolver_key}\n", encoding="utf-8")
...
def run_wizard() -> None:
    ...
    console.print(
        Panel.fit(
            "[bold green]Job Runner Setup Wizard[/bold green]\n\n"
            "This will create your configuration at:\n"
            f"  [cyan]{APP_DIR}[/cyan]\n\n"
            "You can re-run this anytime with [bold]job_runner init[/bold].",
            border_style="green",
        )
    )
    ...
    from job_runner.config import get_tier, TIER_LABELS, TIER_COMMANDS
    ...
    unlock_hint = ""
    if tier == 1:
        unlock_hint = "\n[dim]To unlock Tier 2: configure an LLM API key (re-run [bold]job_runner init[/bold]).[/dim]"
    elif tier == 2:
