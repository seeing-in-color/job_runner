"""Job Runner Pipeline Orchestrator.

Runs pipeline stages in sequence or concurrently (streaming mode).

Usage (via CLI):
    job_runner run                        # all stages, sequential
    job_runner run --stream               # all stages, concurrent
    job_runner run discover enrich        # specific stages
    job_runner run score tailor cover     # LLM-only stages
...
from job_runner.config import load_env, ensure_dirs, load_search_config
from job_runner.database import init_db, get_connection, get_stats
...
def _run_discover(workers: int = 1) -> dict:
    ...
    if run_jobspy:
        ...
        try:
            from job_runner.discovery.jobspy import run_discovery
            run_discovery()
            stats["jobspy"] = "ok"
    ...
    if run_workday:
        ...
        try:
            from job_runner.discovery.workday import run_workday_discovery
    ...
    if run_smart_extract:
        ...
        try:
            from job_runner.discovery.smartextract import run_smart_extract
...
def _run_enrich(workers: int = 1) -> dict:
    ...
    try:
        from job_runner.enrichment.detail import run_enrichment
...
def _run_score(
    ...
    try:
        from job_runner.scoring.scorer import run_scoring
        kw: dict = {
...
def _run_tailor(min_score: int = 7, validation_mode: str = "normal") -> dict:
    ...
    try:
        from job_runner.scoring.tailor import run_tailoring
...
def _run_cover(min_score: int = 7, validation_mode: str = "normal") -> dict:
    ...
    try:
        from job_runner.scoring.cover_letter import run_cover_letters
...
def _run_pdf() -> dict:
    ...
    try:
        from job_runner.scoring.pdf import batch_convert
        batch_convert()
        return {"status": "ok"}
    except Exception as e:
...
def run_pipeline(
    ...
    Args:
        ...
        score_print_profile: If True/False, print or hide one condensed résumé summary at the start of scoring.
            If None, use env JOB_RUNNER_SCORE_PRINT_PROFILE (default: print).
    ...
    console.print(Panel.fit(
        f"[bold]Job Runner Pipeline[/bold] ({mode})",
        border_style="blue",
    ))
